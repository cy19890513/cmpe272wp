 ...
 * 
 */
class bctx_network_admin extends bctx_admin
{
	const version = '6.1.0';
	protected $full_name = 'TX Breadcrumb Network Settings';
	...
	function __construct(bctx_breadcrumb_trail &$breadcrumb_trail, $basename)
	{
		...
		$this->full_name = __('TX Breadcrumb Network Settings', 'tx-breadcrumb');
	...
	function multisite_settings_warn()
	{
		if(is_multisite())
		{
			...
			{
				$this->messages[] = new mtekk_adminKit_message(esc_html__('Warning: Individual site settings will override any settings set in this page.', 'tx-breadcrumb'), 'warning', true, $this->unique_prefix . '_msg_ns_isiteoveride');
			}
			...
			{
				$this->messages[] = new mtekk_adminKit_message(esc_html__('Warning: Individual site settings may override any settings set in this page.', 'tx-breadcrumb'), 'warning', true, $this->unique_prefix . '_msg_ns_isitemayoveride');
			}
			else if(defined('BCN_SETTINGS_FAVOR_NETWORK') && BCN_SETTINGS_FAVOR_NETWORK)
			{
				$this->messages[] = new mtekk_adminKit_message(esc_html__('Warning: Individual site settings may override any settings set in this page.', 'tx-breadcrumb'), 'warning', true, $this->unique_prefix . '_msg_ns_nsmayoveride');
			}
			//Fall through if no settings mode was set
			else
			{
				$this->messages[] = new mtekk_adminKit_message(esc_html__('Warning: No BCN_SETTINGS_* define statement found, defaulting to BCN_SETTINGS_USE_LOCAL.', 'tx-breadcrumb'), 'warning', true, $this->unique_prefix . '_msg_ns_nosetting');
				$this->messages[] = new mtekk_adminKit_message(esc_html__('Warning: Individual site settings will override any settings set in this page.', 'tx-breadcrumb'), 'warning', true, $this->unique_prefix . '_msg_ns_isiteoveride');
			}
		}
	}
