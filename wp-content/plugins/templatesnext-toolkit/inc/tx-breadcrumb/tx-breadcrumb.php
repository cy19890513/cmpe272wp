<?php
/*
Plugin Name: TX Breadcrumb
Plugin URI: http://mtekk.us/code/tx-breadcrumb/
Description: Adds a breadcrumb navigation showing the visitor&#39;s path to their current location. For details on how to use this plugin visit <a href="http://mtekk.us/code/tx-breadcrumb/">TX Breadcrumb</a>. 
Version: 6.1.0
Author: John Havlik
Author URI: http://mtekk.us/
License: GPL2
Text Domain: tx-breadcrumb
...
if(version_compare(phpversion(), '5.3.0', '<'))
{
	...
	{
		printf('<div class="notice notice-error"><p>' . esc_html__('Your PHP version is too old, please upgrade to a newer version. Your version is %1$s, TX Breadcrumb requires %2$s', 'tx-breadcrumb') . '</p></div>', phpversion(), '5.3.0');
	}
...
require_once(dirname(__FILE__) . '/class.bctx_breadcrumb.php');
//Include the breadcrumb trail class
require_once(dirname(__FILE__) . '/class.bctx_breadcrumb_trail.php');
if(class_exists('WP_Widget'))
{
	//Include the WP 2.8+ widget class
	require_once(dirname(__FILE__) . '/class.bctx_widget.php');
}
$tx_breadcrumb = null;
...
class tx_breadcrumb
{
	...
	/**
	 * Constructor for a new tx_breadcrumb object
	 * 
	 * @param bctx_breadcrumb_trail $breadcrumb_trail An instance of a bctx_breadcrumb_trail object to use for everything
	 */
	public function __construct(bctx_breadcrumb_trail $breadcrumb_trail)
	{
		...
		{
			require_once(dirname(__FILE__) . '/class.bctx_network_admin.php');
			//Instantiate our new admin object
			$this->admin = new bctx_network_admin($this->breadcrumb_trail, $this->plugin_basename);
		}
		...
		{
			require_once(dirname(__FILE__) . '/class.bctx_admin.php');
			//Instantiate our new admin object
			$this->admin = new bctx_admin($this->breadcrumb_trail, $this->plugin_basename);
		}
	}
	public function init()
	{
		...
		$this->get_settings();
		add_filter('bctx_allowed_html', array($this, 'allowed_html'), 1, 1);
		...
		{
			require_once(dirname(__FILE__) . '/class.bctx_rest_controller.php');
			$this->rest_controller = new bctx_rest_controller($this->breadcrumb_trail, $this->unique_prefix);
		}
	}
	...
	static public function setup_options(&$opt)
	{
		...
		tx_breadcrumb::find_posttypes($opt);
		//Add custom taxonomy types
		tx_breadcrumb::find_taxonomies($opt);
		//Let others hook into our settings
		$opt = apply_filters('bctx_settings_init', $opt);
	}
	/**
		...
		foreach($wp_post_types as $post_type)
		{
			...
			if(!$post_type->_builtin)
			{
				...
				if(!isset($opts['Hpost_' . $post_type->name . '_template']) || !$post_type->hierarchical && !isset($opts['Spost_' . $post_type->name . '_hierarchy_type']))
				{
					...
					$opts['Hpost_' . $post_type->name . '_template'] = bctx_breadcrumb::get_default_template();
					$opts['Hpost_' . $post_type->name . '_template_no_anchor'] = bctx_breadcrumb::default_template_no_anchor;
					...
					$opts['Spost_' . $post_type->name . '_hierarchy_type'] = apply_filters('bctx_default_hierarchy_type', $opts['Spost_' . $post_type->name . '_hierarchy_type'], $post_type->name);
					$opts['bpost_' . $post_type->name . '_hierarchy_display'] = apply_filters('bctx_default_hierarchy_display', $opts['bpost_' . $post_type->name . '_hierarchy_display'], $post_type->name, $opts['Spost_' . $post_type->name . '_hierarchy_type']);
				}
			}
		}
	}
	...
	static function find_taxonomies(&$opts)
	{
		...
		foreach($wp_taxonomies as $taxonomy)
		{
			...
			if(!$taxonomy->_builtin)
			{
				...
				{
					//Add the necessary option array members
					$opts['Htax_' . $taxonomy->name . '_template'] = __(sprintf('<span property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" title="Go to the %%title%% %s archives." href="%%link%%" class="%%type%%"><span property="name">%%htitle%%</span></a><meta property="position" content="%%position%%"></span>', $taxonomy->labels->singular_name), 'tx-breadcrumb');
					$opts['Htax_' . $taxonomy->name . '_template_no_anchor'] = __(sprintf('<span property="itemListElement" typeof="ListItem"><span property="name">%%htitle%%</span><meta property="position" content="%%position%%"></span>', $taxonomy->labels->singular_name), 'tx-breadcrumb');
				}
			}
		}
	}
	 ...
	 * with one generated by TX Breadcrumb
	 * 
	 * @param string $bradcrumb_trail The string breadcrumb trail that we will replace
	 * @return string The TX Breadcrumb assembled breadcrumb trail
	 */
	...
	private function get_settings()
	{
		...
		$this->breadcrumb_trail->opt = wp_parse_args(get_option('bctx_options'), $this->opt);
		...
		if(is_multisite())
		{
			if(defined('BCN_SETTINGS_USE_NETWORK') && BCN_SETTINGS_USE_NETWORK)
			{
				...
				$this->breadcrumb_trail->opt = wp_parse_args(get_site_option('bctx_options'), $this->opt);
			}
			else if(defined('BCN_SETTINGS_FAVOR_LOCAL') && BCN_SETTINGS_FAVOR_LOCAL)
			{
				//Grab the current settings for the current local site from the db
				$this->breadcrumb_trail->opt = wp_parse_args(get_option('bctx_options'), $this->breadcrumb_trail->opt);
			}
			else if(defined('BCN_SETTINGS_FAVOR_NETWORK') && BCN_SETTINGS_FAVOR_NETWORK)
			{
				//Grab the current settings from the db
				$this->breadcrumb_trail->opt = wp_parse_args(get_site_option('bctx_options'), get_option('bctx_options'));
			}
		}
		...
		//This one isn't needed as it is performed in bctx_breadcrumb_trail::fill(), it's here for completeness only
	...
	public function display($return = false, $linked = true, $reverse = false, $force = false, $template = '%1$s%2$s')
	{
		...
		else
		{
			...
			$credits = "<!-- TX Breadcrumb " . $this::version . " -->\n";
	...
	{
		_deprecated_function( __FUNCTION__, '6.0', 'tx_breadcrumb::display');
...
//Have to bootstrap our startup so that other plugins can replace the bctx_breadcrumb_trail object if they need to
add_action('plugins_loaded', 'bctx_init', 15);
function bctx_init()
{
	global $tx_breadcrumb;
	//Create an instance of bctx_breadcrumb_trail
	$bctx_breadcrumb_trail = new bctx_breadcrumb_trail();
	//Let's make an instance of our object that takes care of everything
	$tx_breadcrumb = new tx_breadcrumb(apply_filters('bctx_breadcrumb_trail_object', $bctx_breadcrumb_trail));
}
/**
 * Outputs the breadcrumb trail
 * 
 ...
 * @return string String-Data of breadcrumb trail.
 */
function bctx_display($return = false, $linked = true, $reverse = false, $force = false)
{
	global $tx_breadcrumb;
	if($tx_breadcrumb !== null)
	{
		return $tx_breadcrumb->display($return, $linked, $reverse, $force);
	}
}
/**
 * Outputs the breadcrumb trail with each element encapsulated with li tags
 ...
 * @return string String-Data of breadcrumb trail.
 */
function bctx_display_list($return = false, $linked = true, $reverse = false, $force = false)
{
	global $tx_breadcrumb;
	if($tx_breadcrumb !== null)
	{
		return $tx_breadcrumb->display($return, $linked, $reverse, $force, "<li%3\$s>%1\$s</li>\n");
	}
}
/**
 * Outputs the breadcrumb trail in Schema.org BreadcrumbList compatible JSON-LD
 ...
 * @return string String-Data of breadcrumb trail.
 */
function bctx_display_json_ld($return = false, $reverse = false, $force = false)
{
	global $tx_breadcrumb;
	if($tx_breadcrumb !== null)
	{
		return $tx_breadcrumb->display_json_ld($return, $reverse, $force);
	}
}
