...
class bctx_widget extends WP_Widget
{
	const version = '6.1.0';
	...
	function __construct()
	{
		...
		$this->allowed_html = apply_filters('bctx_allowed_html', wp_kses_allowed_html('post'));
		...
		$ops = array('classname' => 'widget_tx_breadcrumb', 'description' => __('Adds a breadcrumb trail to your sidebar', 'tx-breadcrumb'));
		parent::__construct('bctx_widget', 'TX Breadcrumb', $ops);
	}
	function widget($args, $instance)
	{
		...
		$instance['pretext'] = apply_filters('bctx_widget_pretext', $instance['pretext'], $instance);
		$title = apply_filters('widget_title', $instance['title'], $instance, $this->id_base);
		//A bit of a hack but we need the DB settings to know if we should exit early
		$opt = get_option('bctx_options');
		...
		if($instance['type'] === 'list')
		{
			...
			bctx_display_list(false, $instance['linked'], $instance['reverse'], $instance['force']);
			echo '</ol>';
		}
		else if($instance['type'] === 'microdata' || $instance['type'] === 'breadcrumblist_rdfa')
		{
			...
			bctx_display(false, $instance['linked'], $instance['reverse'], $instance['force']);
			echo '</div>';
		}
		...
		else
		{
			...
			do_action('bctx_widget_display_trail', $instance);
		}
	...
	function form($instance)
	{
		$instance = wp_parse_args((array) $instance, $this->defaults);?>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"> <?php _e('Title:', 'tx-breadcrumb'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('pretext')); ?>"> <?php _e('Text to show before the trail:', 'tx-breadcrumb'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('type')); ?>"> <?php _e('Output trail as:', 'tx-breadcrumb'); ?></label>
			<select name="<?php echo esc_attr($this->get_field_name('type')); ?>" id="<?php echo esc_attr($this->get_field_id('type')); ?>">
				<option value="list" <?php selected('list', $instance['type']);?>><?php _e('List', 'tx-breadcrumb'); ?></option>
				<option value="microdata" <?php selected('microdata', $instance['type']);?>><?php _e('Schema.org BreadcrumbList (RDFa)', 'tx-breadcrumb'); ?></option>
				<option value="breadcrumblist_microdata" <?php selected('breadcrumblist_microdata', $instance['type']);?>><?php _e('Schema.org BreadcrumbList (microdata)', 'tx-breadcrumb'); ?></option>
				<option value="plain" <?php selected('plain', $instance['type']);?>><?php _e('Plain', 'tx-breadcrumb'); ?></option>
				<?php do_action('bctx_widget_display_types', $instance);?>
			</select>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('linked')); ?>"> <?php _e('Link the breadcrumbs', 'tx-breadcrumb'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('reverse')); ?>"> <?php _e('Reverse the order of the trail', 'tx-breadcrumb'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('front')); ?>"> <?php _e('Hide the trail on the front page', 'tx-breadcrumb'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('force')); ?>"> <?php _e('Ignore breadcrumb cache', 'tx-breadcrumb'); ?></label><br />
		</p>
		<?php
	}
}