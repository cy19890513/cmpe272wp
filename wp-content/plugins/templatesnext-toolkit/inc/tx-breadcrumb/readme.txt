...
TX Breadcrumb, the successor to the popular WordPress plugin Breadcrumb Navigation XT, was written from the ground up to be better than its ancestor. This plugin generates locational breadcrumb trails for your WordPress powered blog or website. These breadcrumb trails are highly customizable to suit the needs of just about any website running WordPress. The Administrative interface makes setting options easy, while a direct class access is available for theme developers and more adventurous users.

= PHP Requirements =
TX Breadcrumb 5.2 and newer require PHP5.3
TX Breadcrumb 5.1.1 and older require PHP5.2
...
* Network admin settings page for managing breadcrumb settings for all subsites with [configurable global priority](http://mtekk.us/archives/guides/controlling-tx-breadcrumb-settings-from-the-network-settings-page/ "Go to the article on configuring the network settings priority.").
* Built in WordPress Widget.
* Extensible via OOP and provided [actions](http://mtekk.us/code/tx-breadcrumb/tx-breadcrumb-doc/2/#action_reference "Go to the TX Breadcrumb Documentation's action reference.") and [filters](http://mtekk.us/code/tx-breadcrumb/tx-breadcrumb-doc/2/#filter_reference "Go to the TX Breadcrumb Documentation's filter reference.").
...
TX Breadcrumb now supports WordPress.org language packs. Want to translate TX Breadcrumb? Visit [TX Breadcrumb's WordPress.org translation project](https://translate.wordpress.org/projects/wp-plugins/tx-breadcrumb/).

== Installation ==
TX Breadcrumb can be installed from within WordPress’ administration panel. After installing and activating the plugin, to get breadcrumb trails to display either use the included widget, or call the breadcrumb trail in your theme (or child theme). See the [Calling the Breadcrumb Trail](http://mtekk.us/archives/guides/calling-the-breadcrumb-trail "Read more on calling the breadcrumb trail") article for more information on calling the breadcrumb trail.

To customize the breadcrumb trail you may edit the default values for the options in the administrative interface. This is located in your administration panel under Settings > TX Breadcrumb.

Please visit [TX Breadcrumb's Documentation](http://mtekk.us/code/tx-breadcrumb/tx-breadcrumb-doc/ "Go to TX Breadcrumb's Documentation.") page for more information.

== Screenshots ==
1. This screenshot shows 5 different examples of breadcrumbs generated by TX Breadcrumb
...
* Behavior change: Added auto migration of post type hierarchy settings to `bctx_display*()` functions.
...
* Behavior change: Added warning alerting that `bctx_breadcrumb::type` must be an array.
...
* Bug fix: Fixed issue where the `$reverse` parameter for `bctx_display` and `bctx_display_list` did not work properly.
* Bug fix: Fixed issue where the `bctx_display_list` function did not include the `li` elements.
...
* Behavior change: `bctx_breadcrumb_trail::display_list()` deprecated in favor of using the `$template` parameter in `bctx_breadcrumb_trail::display()`.
* Behavior change: `bctx_breadcrumb_trail::do_attachment()` deprecated in favor of calling `bctx_breadcrumb_trail::do_post()`.
* Behavior change: `bctx_breadcrumb_trail::do_front_page()` deprecated in favor of calling `bctx_breadcrumb_trail::do_home()`.
* Behavior change: `bctx_li_attributes` filter was deprecated in favor of `bctx_display_attributes`.
* Behavior change: `bctx_breadcrumb_trail::do_archive_by_date()` deprecated in favor of calling bctx_breadcrumb_trail::do_day()`, `bctx_breadcrumb_trail::do_month()`, and/or `bctx_breadcrumb_trail::do_year()`.
* Behavior change: `bctx_breadcrumb_trail::find_type()` deprecated and removed from bctx_breadcrumb_trail.
...
* New feature: Added `bctx_default_hierarchy_display` filter.
* New feature: Added `bctx_default_hierarchy_type` filter.
* New feature: Added `$posttype_name` as the third parameter to `bctx_show_tax_private`.
...
* Bug fix: Fixed erroneous use of `$linked` that caused a PHP warning in `bctx_display_json_ld()`.
...
* New feature: Added `bctx_display_json_ld(`) function for producing Schema.org BreadcrumbList compliant JSON-LD markup.
* Bug fix: Fixed issue where the uninstaller caused PHP Errors when ran from WP CLI.
* Bug fix: Fixed issue where `bctx_breadcrumb_trail::find_type()` caused a PHP Warnings in certain circumstances.
* Bug fix: Fixed typo in the administrative interface.

= 5.6.0 =
Release date: December 23rd, 2016

* New feature: Added fourth parameter, `$force`, to `bctx_display()`, and `bctx_display_list()` allowing the internal caching mechanism to be bypassed.
...
* Bug fix: Fixed issue in `bctx_breadcrumb_trail::find_type()` that identified pages as posts, causing the erroneous inclusion of the post root in the breadcrumb trail for pages. 
...
* Behavior change: Taxonomy term hierarchy selection logic in `bctx_breadcrumb_trail::pick_post_term()` has changed to picking the deepest known child of the first hierarchy found in the list of terms returned by `get_the_terms()` rather than the first term with a child.
...
* New feature: Added `bctx_post_terms` filter to `bctx_breadcrumb_trail::post_terms()` to control the terms included in a non-hierarchical term breadcrumb for a post.
* New feature: Added `bctx_add_post_type_arg` filter to `bctx_breadcrumb_trail::maybe_add_post_type_arg()` to control when the post_type argument is added to URLs for archives.
* New feature: Added `bctx_pick_post_term` filter to `bctx_breadcrumb_trail::post_hierarchy()` to allow overriding TX Breadcrumb’s default term selection behavior.
* Bug fix: Fixed issue with untranslatable title on the settings page.
* Bug fix: Cleanup of several trivial differences between `bctx_admin` and `bctx_network_admin`.
...
* Bug fix: Added translation wrappers for date format strings in `bctx_breadcrumb_trail::do_archive_by_date()`.
* Bug fix: Fixed issue where `bctx_breadcrumb_trail::is_builtin()` would cause PHP warnings when the passed in post type was not an actual post type.
...
* Bug fix: Fixed PHP error caused by `bctx_breadcrumb_trail::get_type_string_query_var()` returning an array when the post_type query variable is an array.

= 5.3.0 =
Release date: November 12th, 2015

* Behavior change: TX Breadcrumb will no longer default to setting the root page for CPTs.
* Behavior change: TX Breadcrumb will no longer inject a breadcrumb for a CPT archive page if a root page is set for that CPT.
* Behavior change: TX Breadcrumb now defaults to `BCN_SETTINGS_USE_LOCAL` rather than `BCN_SETTINGS_USE_NETWORK` if all of the `BCN_SETTINGS_*` globals are not defined.
...
* New feature: Added `bctx_widget_display_trail` action to enhance the included widget’s extensibility.
* New feature: Added `bctx_widget_display_types` action to enhance the included widget’s extensibility.
* New feature: The plugin uninstaller has been re-factored, includes support for uninstalling in PHP5.2.
* New feature: Unit tests added for all non-deprecated features in bctx_breadcrumb.
...
* Bug fix: Added additional checks for empty URLs and templates within `bctx_breadcrumb` to prevent various bugs when empty URLs and/or templates are passed in.
...
* New feature: Added `bctx_type_archive_post_type` filter.
...
* Behavior change: `bctx_breadcrumb_trail::do_post()` now expects to be passed in a valid WP_Post object.
* Behavior change: `tx_breadcrumb::version` changed to a constant to allow uninstantiated access of the plugin version.
* New feature: Support Google's RDFa Breadcrumbs microformat by default.
* New feature: Added `bctx_opts_update_prebk` filter.
...
* Bug fix: Added ID to Post and Taxonomy Term Elements in `bctx_breadcrumb_trail::do_root()` as is done everywhere else.
* Bug fix: Fixed issue with `bctx_breadcrumb_trail::do_author()` returning the incorrect user under some circumstances.
...
* Bug fix: Added bctx_breadcrumb_template filter back into `bctx_breadcrumb::set_template()`, was a regression in 5.0 from 4.4.
...
* Bug fix: Fixed issue where `bctx_breadcrumb_trail::display_list()` would produce multiple instances of the class attribute.
* Bug fix: Fixed several issues with the uninstaller.

= 5.0.0 =
Release date: November 20th, 2013

* Behavior Change: Moved `bctx_breadcrumb_trail::trail` to `bctx_breadcrumb_trail::breadcrumbs`
...
* New feature: Added `bctx_breadcrumb_trail_object` filter.
* New feature: Added `bctx_li_attributes` filter.
* New feature: Added `bctx_breadcrumb_types` filter.
...
= 5.6.0 =
This version requires PHP5.3 or newer. This version introduces a new 4th optional parameter to `bctx_display()` and `bctx_display_list()` that bypasses the internal caching mechanism.