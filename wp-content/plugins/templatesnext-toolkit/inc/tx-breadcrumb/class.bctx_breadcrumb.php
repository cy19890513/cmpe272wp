...
class bctx_breadcrumb
{
	...
	public function __construct($title = '', $template = '', array $type = array(), $url = '', $id = null)
	{
		...
		$this->template_no_anchor = bctx_breadcrumb::default_template_no_anchor;
		//If we didn't get a good template, use a default template
		if($template == null)
		{
			$this->set_template(bctx_breadcrumb::get_default_template());
		}
		...
		else
		{
			...
			if($url == null)
			{
				$this->template_no_anchor = wp_kses(apply_filters('bctx_breadcrumb_template_no_anchor', $template, $this->type, $this->id), $this->allowed_html);
				$this->set_template(bctx_breadcrumb::get_default_template());
			}
			else
			{
	...
	{
		return __('<span property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" title="Go to %title%." href="%link%" class="%type%"><span property="name">%htitle%</span></a><meta property="position" content="%position%"></span>', 'tx-breadcrumb');
	}
	/**
	...
	{
		//Set the title
		$this->title = apply_filters('bctx_breadcrumb_title', $title, $this->type, $this->id);
	...
	{
		$url = trim($url);
		$this->url = apply_filters('bctx_breadcrumb_url', $url, $this->type, $this->id);
	...
	public function set_template($template)
	{
		...
		$this->template = wp_kses(apply_filters('bctx_breadcrumb_template', $template, $this->type, $this->id), $this->allowed_html);
	}
	/**
	...
	public function assemble($linked, $position)
	{
		...
		$replacements = array(
			...
			'%htitle%' => $this->title,
			'%type%' => apply_filters('bctx_breadcrumb_types', $this->type, $this->id),
		...
		{
			_doing_it_wrong(__CLASS__ . '::' . __FUNCTION__, __('bctx_breadcrumb::type must be an array', 'tx-breadcrumb'), '6.0.2');	
		}
		$replacements = apply_filters('bctx_template_tags', $replacements, $this->type, $this->id);
